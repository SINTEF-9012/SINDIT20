...
def update_propery_node(node: AbstractAssetProperty):
    ...
    #Warning: connection has to be created before the property. Otherwise, the property will not be attached to the connection
    ...
    if str(node.type).lower() == "mqtt":
        ...
        try:
            if node_uri not in connections:
                ...
                connection = connections[node_uri]
                #Warning: updating the username and password does not create a new connection
                if str(connection.host) != str(node.host) or str(connection.port) != str(node.port):
                    ...
                    connection.start()
                elif node.isConnected is None or not node.isConnected or str(node.isConnected).lower() == "false":
                    connection.stop()
                    connection.start()
...
def initialize_connections_and_properties():
    ...
    for node in sindit_kg_connector.load_nodes_by_class(AbstractAssetProperty.CLASS_URI):
